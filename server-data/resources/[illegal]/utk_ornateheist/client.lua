...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Banka soygununu baslat",
        silentstart = "[~r~E~w~] Banka soygununu gizlice baslat",
        usecard = "[~r~E~w~] Banka calısan kartı kullan",
        usethermal = "[~r~E~w~] Termal sarj kullan",
        usehack = "[~r~E~w~] Güvenlik panelini hackle",
        uselockpick = "[~r~E~w~] Maymuncuk kullan",
        usesearch = "[~r~E~w~] Ara",
        lootcash= "[~r~E~w~] Paraları topla",
        lootgold = "[~r~E~w~] Altınları topla",
        lootdia = "[~r~E~w~] Elmasları topla",
        card = "Banka çalışan kartı kullanılıyor",
        thermal = "Termal şarj kuruluyor",
        burning = "Kapı kilidi eriyor",
        lockpick = "Kapı açılıyor",
        using = "Panele işleniyor",
        used = "Sürec tamamlandı",
        stage = "Tamamlandı.",
        search = "Aranıyor",
        hacking = "Hackleniyor",
        melted = "Kapı kilidi eridi",
        hacked = "Hack tamamlandı",
        unlocked = "Kapı kırıldı.",
        nothing = "Hicbir sey bulunamadi",
        found = "Kimlik karti buldun",
        time = "Kalan: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Hack için laptop lazım")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Banka çalışan kimliği lazım")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Termal şarjın yok")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "Bilgisayarınızı açın ve HackConnect.exe'ye gidin")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Zehirli gaz tespit edildi salınmasına 160 saniye kaldı.")
        UTK.starttimer = true
        UTK.vaulttime = 160
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:SendAlert("success", "Çaldın "..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Zehirli gaz tespit edildi salınmasına 90 saniye kaldı.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "Hazine kapısı 60 saniye içinde kapanacak")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "Banka Kapısı Kapanıyor!!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("error", "RiaV Merkez Bankası alarmı tetiklendi. Biri soyuyor olabilir harekete geç ! ", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
