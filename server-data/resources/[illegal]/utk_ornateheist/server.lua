...
local mincash = 1700 -- minimum amount of cash a pile holds
local maxcash = 2000-- maximum amount of cash a pile can hold
...
local cooldown = 0 -- amount of time to do the heist again in seconds (30min)
...
ESX.RegisterServerCallback("utk_oh:startevent", function(source, cb, method)
    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." Yeni soygun için kalan planlanmış süre")
        end
    else
        cb("Banka zaten soyuldu")
    end
end)
