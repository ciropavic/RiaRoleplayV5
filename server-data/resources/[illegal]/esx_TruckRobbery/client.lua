...
function AtmHackSuccess(success)
    ...
    if success then
		...
		exports['mythic_notify']:SendAlert('error',Config.HackingFailed)
...
AddEventHandler("esx_TruckRobbery:startMission",function(spot)
	...
	if numy == 100 then
		exports['mythic_notify']:SendAlert('error',Config.NoMissionsAvailable)
	else
		CurrentEventNum = num
		TriggerEvent("esx_TruckRobbery:startTheEvent",num)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		exports['mythic_notify']:SendAlert('success', Config.TruckMarkedOnMap)
	end
end)
...
AddEventHandler('esx_TruckRobbery:startTheEvent', function(num)
	...
	while not taken do
		...
		if missionInProgress == true then
			...
			if distance <= 30.0  then
				if KillGuardsText == false then
					exports['mythic_notify']:SendAlert('error',Config.KillTheGuards)
					KillGuardsText = true
				end
			end
		...
		if missionCompleted == true then
			exports['mythic_notify']:SendAlert('success',Config.MissionCompleted)
...
function BlowTheTruckDoor()
	if IsVehicleStopped(ArmoredTruckVeh) then
		if (IsVehicleSeatFree(ArmoredTruckVeh, -1) and IsVehicleSeatFree(ArmoredTruckVeh, 0) and IsVehicleSeatFree(ArmoredTruckVeh, 1)) then
			...
			exports['mythic_notify']:SendAlert('error', Config.BeginToRobTruck)
		else
			exports['mythic_notify']:SendAlert('error', Config.GuardsNotKilledYet)
		end
	else
		exports['mythic_notify']:SendAlert('error', Config.TruckIsNotStopped)
	end
end
