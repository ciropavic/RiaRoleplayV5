...
AddEventHandler('esx_TruckRobbery:missionAccepted', function()
	...
	if not CheckCooldownTimer(GetPlayerIdentifier(source)) then
		...
		if accountMoney <= Config.MissionCost then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text =  Config.NotEnoughMoney})
		else
			for i = 1, #Players do
			...
			if policeOnline >= Config.RequiredPoliceOnline then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text =  Config.NotEnoughPolice})
			end
		end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = string.format(Config.CooldownMessage,GetCooldownTimer(GetPlayerIdentifier(source)))})
	end
end)
...
AddEventHandler('esx_TruckRobbery:missionComplete', function()
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text =  string.format(Config.RewardMessage,reward)})
	...
	if Config.EnableItemReward == true then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text =  string.format(Config.Item1Message,itemAmount1)})
	end
	...
	if Config.EnableRareItemReward == true then
		...
		if chance == 1 then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text =  string.format(Config.Item2Message,itemAmount2)})
		end	
	end
