	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("~W, A, S, D ~ s ~ ile gezinin ve sol kod bloğu için ~ y ~ SPACE ~ s ~ ile onaylayın.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Doğru kod bloğu için ~ y ~ Ok Tuşları ~ s ~ ve ~ y ~ ENTER ~ s ~ kullanın")
			else
