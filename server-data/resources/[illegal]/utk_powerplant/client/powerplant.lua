...
UTK = {
    ...
    texts = {
        blackout = "KARANLIK!!!",
        loud = {
            start = "[~g~E~w~] Elektrik kesintisi için patlatmaya basla",
            bomb = "[~g~E~w~] Elektrik jenatörü için C4 patlayıcılarını kur",
            backup = "[~g~E~w~]  Yedek generator için c4 patlayıcılarını kur",
            time = "Karanlığa 200 saniye!",
            planted = "C4 kuruldu"
        },
    },
    npc = {
                                ...
                                if bomb1 <= 1.5 and IsControlJustReleased(0, 38) then
                                    ESX.TriggerServerCallback("utk_pb:checkItem", function(cb)
                                        if cb then
                                            ...
                                            self:PlantBomb()
                                        elseif not cb then
                                            exports['mythic_notify']:SendAlert("error", "C4 patlayıcın bulunmuyor.")
                                        end
                                    end, "normal_c4")
                                end
                            end
                                ...
                                if hack1 <= 1.5 and IsControlJustReleased(0, 38) then
                                    ESX.TriggerServerCallback("utk_pb:checkItem", function(cb)
                                        if cb then
                                            ...
                                            self:Hack()
                                        elseif not cb then
                                            exports['mythic_notify']:SendAlert("error", "Hacker bilgisayarına sahip değilsin.")
                                        end
                                    end, "laptop_h")
                                end
                            end
                                ...
                                if hack5 <= 1.5 and IsControlJustReleased(0, 38) then
                                    ESX.TriggerServerCallback("utk_pb:checkItem", function(cb)
                                        if cb then
                                            ...
                                            self:Hack()
                                        elseif not cb then
                                            exports['mythic_notify']:SendAlert("error", "Mini C4 e sahip değilsin.")
                                        end
                                    end, "mini_c4")
                                end
                            end
...
function UTK:PlantBomb(...)
    ...
    exports['progressBars']:startUI(4500, "Kuruluyor")
    Citizen.Wait(1500)
    ...
    exports['mythic_notify']:SendAlert("success", "C4 patlayıcısı kuruldu")
...
function UTK:Blackout(...)
    ...
    exports['mythic_notify']:SendAlert("success", "KARANLIK !!!")
end
...
AddEventHandler("utk_pb:power", function(status)
    ...
    if not status then
        exports['mythic_notify']:SendAlert("success", "Güç kaynakları devlet tarafından onarıldı tekrardan ışıklar aktif")
    elseif status then
        UTK.info.locked = true
    end
end)
...
UTKreset = {
    ...
    texts = {
        ...
        loud = {
            ...
            planted = "C4 patlayıcısı kuruldu"
        },
        silent = {
