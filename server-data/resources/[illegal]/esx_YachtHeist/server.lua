...
AddEventHandler('esx_YachtHeist:safeRobbed', function(id, state)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = "Çaldın "..cashReward.." $ ve "..itemReward.."adet Altın Saat"})
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getHackerDevice",function(source,cb)
	...
	if xPlayer.getInventoryItem(Config.HackerDevice).count >= 1 then
		...
		cb(false)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text =  "Terminali hacklemek için hacker bilgisayarına ithiyacın var"})
	end
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getDrillItem",function(source,cb)
	...
	if xPlayer.getInventoryItem(Config.DrillItem).count >= 1 then
		...
		cb(false)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Drill'e ihtiyacın var!"})
	end
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getCooldownHeist",function(source,cb)
	...
	if YachtHeist[1].recentlyRobbed == false then
		...
		cb(false)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = string.format("Yeni soygunu yapabilmek için kalan süre",GetCooldownTimer())})
	end
end)
