...
Citizen.CreateThread(function()
	if Config.EnableSmelteryBlip == true then
	  for k,v in ipairs(Config.GoldSmeltery)do
		...
		AddTextComponentString("Altın Dökümhanesi")
...
Citizen.CreateThread(function()
	if Config.EnableExchangeBlip == true then
	  for k,v in ipairs(Config.GoldExchange)do
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Altın Borsası")
		EndTextCommandSetBlipName(blip)
	  end
	end
end)
	...
	while true do
		...
		for k,v in pairs(Config.GoldSmeltery) do
			...
			if distance <= 1.5 and meltingGold == false then
				DrawText3Ds(v.x, v.y, pos.z, "Altınları eritmek icin ~g~[E]~s~ tusuna bas")
	...
	while true do
		...
		for k,v in pairs(Config.GoldExchange) do
			...
			if distance <= 0.6 then
				DrawText3Ds(v.x, v.y, pos.z, "Altınları paraya dönüstürmek icin ~g~[E]~s~ tusuna bas")
				if IsControlJustPressed(0, 38) then
...
AddEventHandler("GoldWatchToGoldBar", function()
	...
	exports['progressBars']:startUI((Config.SmelteryTime * 1000), "ALTIN SAAT > ALTIN BAR")
...
AddEventHandler("GoldBarToCash", function()
	...
	--TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
	exports['progressBars']:startUI((Config.ExchangeTime * 1000), "ALTIN BARLARI BORSAYA AKTARILIYOR")
	...
	converting = false
	exports['mythic_notify']:SendAlert('success', "Paranızı dönüştürdünüz!")
end)
