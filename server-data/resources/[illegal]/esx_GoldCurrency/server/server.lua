...
ESX.RegisterServerCallback("esx_goldCurrency:getGoldJobCoolDown",function(source,cb)
	...
	if not CheckGoldJobTimer(GetPlayerIdentifier(source)) then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = string.format("Bidahaki soygunu yapabilmek icin gecmesi gereken süre",GetGoldJobTimer(GetPlayerIdentifier(source)))})
		cb(true)
	end
end)
...
ESX.RegisterServerCallback("esx_goldCurrency:getPayment",function(source,cb)
	...
	if Config.UseBlackMoneyAsMissionCost == true then
		if blackMoney <= Config.MissionCost then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Bu iş için ücret ödemek için yeterli paraya sahip değilsiniz"})
			cb(false)
		else
		...
		if moneyCash <= Config.MissionCost then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = "Bu iş için ücret ödemek için yeterli paraya sahip değilsiniz"})
			cb(false)
		else
...
ESX.RegisterServerCallback("esx_goldCurrency:getMissionavailability",function(source,cb)
	...
	if policeOnline >= Config.RequiredPoliceOnline then
		...
		cb(false)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Sehirde yeterli polis yok"})
	end
end)
...
AddEventHandler('esx_goldCurrency:reward', function()
	...
	if Config.EnableSecondItemReward == true and SecondItem == true then
		...
		if Config.EnableCustomNotification == true then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Görev Tamamlandı. Çaldın :"..itemAmount1.."adet "..item1.." ve "..itemAmount2.."adet "..item2})
		end
	else
		xPlayer.addInventoryItem(Config.ItemName1,itemAmount1)
		if Config.EnableCustomNotification == true then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Görev Tamamlandı. Çaldın "..itemAmount1.."adet "..item1})
		end
	end
...
AddEventHandler('esx_goldCurrency:GoldJobInProgress', function(targetCoords, streetName)
	TriggerClientEvent('esx_goldCurrency:outlawNotify', -1,string.format("^3 Silah sesleri duyuldu ^0 ve ^5%s^0 soygun devam ediyor ",streetName))
	...
	if xPlayer.getInventoryItem("goldwatch").count >= 100 then
		if xPlayer.getInventoryItem("goldbar").count <= 99 then
			if not CheckIfMelting(GetPlayerIdentifier(source)) then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = string.format("Zaten bir işiniz var!",GetTimeForMelting(GetPlayerIdentifier(source)))})
			end
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Altın külçeler için yeterli boş alanınız yok"})
		end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Bir altın barı için 100 adet saat gerekir"})
	end
end)
...
AddEventHandler('esx_goldCurrency:goldExchange', function()
	...
	if not CheckIfExchanging(GetPlayerIdentifier(source)) then
		if xPlayer.getInventoryItem("goldbar").count >= 70 then
			...
			xPlayer.addInventoryItem('cash', 70000)
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "En az 70 adet altın barına ihtiyacınız var"})
		end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = string.format("Altın dönüşümü yapabilirsiniz şu süreç içinde => ",GetTimeForExchange(GetPlayerIdentifier(source)))})
	end
end)
