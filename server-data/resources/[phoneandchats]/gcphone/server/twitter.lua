...
ESX.RegisterServerCallback('gcphone:getTwitterUsers', function(source, cb, username, password)

  local e = getUsers(username, password)
  print(e.id)
  MySQL.Async.fetchAll('SELECT twitter_tweets.*, twitter_accounts.username as author, twitter_accounts.avatar_url as authorIcon, twitter_likes.id AS isLikes FROM twitter_tweets LEFT JOIN twitter_accounts ON twitter_accounts.id = @accountId LEFT JOIN twitter_likes  ON twitter_tweets.id = twitter_likes.tweetId AND twitter_likes.authorId = @accountId WHERE twitter_tweets.authorId = @accountId ORDER BY TIME DESC LIMIT 30',{["@accountId"]=e.id},function(result)
...
function getUsers(username, password)
	local result = MySQL.Sync.fetchAll("SELECT * FROM twitter_accounts WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {['@username'] = username, ['@password'] = password})
...
AddEventHandler('gcPhone:twitter_newTweets', function (tweet)
  ...
  local discord_webhook = GetConvar('discord_webhook', Config.TwitterWeb)
