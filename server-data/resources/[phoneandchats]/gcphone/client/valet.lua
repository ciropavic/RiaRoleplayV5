...
RegisterNUICallback('getCarsValet', function(data, cb)
	...
	ESX.TriggerServerCallback('erp_garage:checkMoney2', function(hasEnoughMoney)
        --if hasEnoughMoney == true then
          --  SpawnVehicle({data,nil},true)
		if hasEnoughMoney == "deudas" then
			ESX.ShowNotification(_U('vale_checmoney'))
        else
    TriggerServerEvent("valet:setOutside", props.plate)
	...
	while not HasModelLoaded(vehhash) and not HasModelLoaded(driverhash) do
  	...
  	if found and HasModelLoaded(driverhash) then
		ESX.Game.SpawnVehicle(vehicle.model, {
			...
			TriggerEvent('disc-hotwire:forceTurnOver', callback_vehicle)
...
function GiveKeysTakeMoney()
	...
	--DeleteEntity("prop_anim_cash_note_b")
	TaskWanderStandard(fizzPed, 10.0, 10)
