...
function GenerateUniquePhoneNumber()
    ...
    while running do
		...
		local rand = string.format("%04d", numBase )
...
AddEventHandler('esx_cartesim:sim_give', function (store, index)
  ...
  TriggerClientEvent('esx:showNotification', id, "Sim kartı aldınız~o~" .. simZ)
	...
	Citizen.Wait(500)
	TriggerEvent('gcPhon:onPlayerLoaded', _source) 
...
RegisterServerEvent('esx_cartesim:sim_cal')
AddEventHandler('esx_cartesim:sim_cal', function (index)
  local _source = source
  local id = index
  local xPlayer = ESX.GetPlayerFromId(_source) 	
  local xPlayer2 = ESX.GetPlayerFromId(id)		
  ...
  TriggerClientEvent('esx:showNotification', _source, "You took the sim card " )
  TriggerClientEvent('esx:showNotification', id, "Your sim cards have been stolen" )
	...
	if simZ == 'numara' then
		...
		MySQL.Async.execute(
			'UPDATE sim SET identifier = @identifier WHERE identifier = @identifier2',
			{
				['@identifier'] = xPlayer.identifier,
				['@identifier2'] = xPlayer2.identifier
			}
		)
		...
		Citizen.Wait(500)
		TriggerEvent('gcPhon:onPlayerLoaded', _source) 
		TriggerEvent('gcPhon:onPlayerLoaded', id) 
