...
RegisterServerEvent('fu_chat:server:do')
AddEventHandler('fu_chat:server:do', function(name, message)
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div class="chat-message do">  {0} | {1}</div>',
        args = { name, message }
    })
    CancelEvent()
end)

RegisterServerEvent('fu_chat:server:me')
AddEventHandler('fu_chat:server:me', function(name, message)
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div class="chat-message me">  {0} | {1}</div>',
        args = { name, message }
    })
    CancelEvent()
end)
...
AddEventHandler('fu_chat:server:Advert', function(name, message)
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div class="chat-message advert"><i class="fas fa-ad"></i> Reklam: {0} | {1}</div>',
        args = { name, message }
    })
    CancelEvent()
end)
