...
function JailLogin()
	...
	exports['mythic_notify']:SendAlert('error', 'En son rüyaya daldığınızda hapisteydiniz  bu yüzden hapiste uyandınız')
...
function UnJail()
	...
	exports['mythic_notify']:SendAlert('success', 'Serbest bırakıldın iyi şanslar')
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			exports['mythic_notify']:SendAlert('error', 'Hapishanede' ..jailTime.. ' dakikanız kaldı!')
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Paketi"

					if not v["state"] then
						PackageText = "Zaten alınmış"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								exports['mythic_notify']:SendAlert('error', 'Bu paketi zaten aldın')
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Açık Kapı", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Ambalajlanıyor " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		...
		{
			title    = "Hapishane Menu",
			align    = 'center',
			elements = {
				{ label = "Federale Götür", value = "jail_closest_player" },
				{ label = "Federalden mahkum serbest bırak", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
					exports['mythic_notify']:SendAlert('error', 'Zamanın dakikalar içinde dolması bekleniyor')
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Yakında şehirli yok!")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								exports['mythic_notify']:SendAlert('error', 'Buraya birşey koymanız gerekiyor')
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									exports['mythic_notify']:SendAlert('success', 'Yakında şehirli yok')
								else
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					exports['mythic_notify']:SendAlert('error', 'Hapishaneniz boş')
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Mahkum: " .. playerArray[i].name .. " | Hapishane: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end
