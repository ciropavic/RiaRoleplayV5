RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		exports['mythic_notify']:SendAlert('error', 'SEN POLİS DEĞİLGSİN!')
	end
end)
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Hapishane" or action == "Güvenlik" then

				if PlayerData.job.name ~= "police" then
					exports['mythic_notify']:SendAlert('error', 'Buraya gitmek için anahtarınız yok')
					return
				end
			end
