	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text =  GetPlayerName(jailPlayer) .. " Hapise atıldın" .. jailTime .. "dakika"})
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "HAKİM",  Firstname .. " " .. Lastname .. "Şu anda hapishanede: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text =  "Bu sefer geçersiz!"})
			end
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text =  "Bu kimlik çevrimiçi değil!"})
		end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text =  "Görevli Değilsin"})
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "Hakim",  Firstname .. " " .. Lastname .. " Şimdi bu nedenle hapiste: " .. jailReason }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text =  GetPlayerName(targetSrc) .. " hapise atıldın " .. jailTime .. " dakika!"})
end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " Serbest Bırakıldı")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Teşekkürler, burada yemek için biraz nakit var!")
end)
