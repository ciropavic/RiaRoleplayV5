...
local Positions = {
	...
	['Sell'] = { ['hint'] = '[E] Satış', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
	...
	Citizen.CreateThread(function()
		for index, v in ipairs(Positions) do
			if index ~= 'SpawnATV' then
				...
				AddTextComponentString('Avlanma Bölgesi')
...
function StartHuntingSession()
    ...
    if OnGoingHuntSession then
        ...
        TriggerServerEvent('hunting:removeWeapons')
		...
		TriggerServerEvent('hunting:addWeapons')
		SetCurrentPedWeapon(GetPlayerPed(-1), GetHashKey('WEAPON_MUSKET'), true)
        SetPedAmmo(GetPlayerPed(-1), GetHashKey("WEAPON_MUSKET"), 50)
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Hayvanı Kes', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_HATCHET')  then
										...
										TriggerEvent('mythic_notify:client:SendAlert', {type='success', text='Bıçağını eline almalısın önce bir bıçağın olmalı'})
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	TriggerEvent('mythic_notify:client:SendAlert', {type='success', text='Hayvanı kestin ' ..AnimalWeight.. ' kg et aldın'})
