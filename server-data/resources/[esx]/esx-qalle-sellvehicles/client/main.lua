		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "[E] tusuna bas ve ikinci el satısa ulas", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							exports['mythic_notify']:SendAlert('error', "Arabanızın içinde olmalısınız!")
						end
					end
				end
			end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			table.insert(elements, { ["label"] = "Fiyatını Degistir - " .. price .. " :-", ["value"] = "price" })
			table.insert(elements, { ["label"] = "Satıs icin koy", ["value"] = "sell" })
		else
			table.insert(elements, { ["label"] = "Fiyatını Olustur- :-", ["value"] = "price" })
		end
	else
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					exports['mythic_notify']:SendAlert('success', "Aracızı satışa koydunuz Fiyatı : " .. price .. " :-")
					menu.close()
				else
					exports['mythic_notify']:SendAlert('error', "Aracınızı satmak için kendi aracınızı satışa sunmanız gerekir " .. #Config.VehiclePositions)
				end
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					exports['mythic_notify']:SendAlert('success', "Araç satın aldınız ve " .. price .. " ödediniz!")
					menu.close()
				else
					exports['mythic_notify']:SendAlert('error', "Yeterli paranız yok, Eksik fiyat: " .. price - totalMoney)
				end
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					exports['mythic_notify']:SendAlert('success',"Aracınızı satıştan başarıyla geri aldınız")
					menu.close()
				end
