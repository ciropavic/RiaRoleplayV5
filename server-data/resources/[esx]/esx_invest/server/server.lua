...
AddEventHandler("invest:buy", function(job, amount, rate)
    ...
    if(amount == nil or amount <= 0) then
        return   TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'success', text='Geçersiz Miktar'})
    elseif(Config.Stock.Limit ~= 0 and amount > Config.Stock.Limit) then
        ...
        if(bank < amount) then
            return   TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'error', text='Yeterli miktarda paran yok!'})
        end
    ...
    if(type(inf) == "table" and inf.job ~= nil) then
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'success', text='Senetine daha fazla yatırım yaptın'})
    else
        if Config.Debug then
        ...
        if rate == nil then
            return   TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'error', text='Beklenmeyen bir hata oluştu'})
        end
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'success', text='Bir yatırım yaptınız umarım kazancınız güzel olur İyi günler.'})
    end
...
AddEventHandler("invest:sell", function(job)
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = 'success', text='Hisse senetiniz satıldı. İyi yatırımdı teşekkürler'})
    TriggerEvent(_source, "invest:balance")
end)
