...
ESX.RegisterUsableItem('bag', function(source)
    ...
    if not HasBag then
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'error', text ='Çantan zaten var'})
    end
end)
...
AddEventHandler('esx-kr-bag:PutItem', function(id, item, label, count, type)
    ...
    elseif type == 'item' and count < Config.MaxItemCount then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_bag_inventory WHERE id = @id ',{["@id"] = id}, function(result)
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', src, {type='error', text ='Sadece ' .. Config.MaxItemCount .. ' eşyaya sahip olabilirsin'})
        end
    end)
end)
...
AddEventHandler('esx-kr-bag:PickUpBag', function(id)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', src, {type = 'inform', text = 'Çantana ulasmak icin [F5] tuşuna bas'})
        TriggerClientEvent('esx-kr-bag:ReSync', xPlayers[i], id)
     end
end)
