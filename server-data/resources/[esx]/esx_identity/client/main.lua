		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Kabul edilemez boy oranı!"
					break
				end
			else
				reason = "Kabul edilemez boy oranı!"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Sehir adınız ya cok uzun ya cok kısa.'
	end
	...
	if count ~= nameLength then
		return 'Sehir adiniz special karakterler icermektedir buna izin verilmez'
	end
	...
	if spacesInName > 2 then
		return 'İsminiz cok fazla bosluk iceriyor'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'İsminiz büyük harfle baslamali'
	end
