		...
		if(data.current.value == 'confirm_purchase') then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_choose_name_dialog', {
				...
				ESX.TriggerServerCallback('t1ger_deliveryjob:buyCompany', function(purchased)
					if purchased then
						exports['mythic_notify']:SendAlert('success', (Lang['company_bought']):format(math.floor(val.price)))
		...
		if(data.current.value == 'rename_company') then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_rename_dialog', {
				...
				ESX.TriggerServerCallback('t1ger_deliveryjob:renameCompany', function(renamed)
					if renamed then
						exports['mythic_notify']:SendAlert('success', ((Lang['company_renamed']):format(firmName)))
						TriggerServerEvent('t1ger_deliveryjob:fetchCompanySV')
						menu.close()
						deliveryMenu = nil
					else
						exports['mythic_notify']:SendAlert('error', (Lang['not_your_company']))
						menu.close()
						deliveryMenu = nil
					end
				end, id, val, firmName)
	...
	function(data, menu)
		if(data.current.value == 'confirm_sale') then
			ESX.TriggerServerCallback('t1ger_deliveryjob:sellCompany', function(sold)
				if sold then
					TriggerServerEvent('t1ger_deliveryjob:fetchCompanySV')
					exports['mythic_notify']:SendAlert('success', (Lang['company_sold']):format(math.floor(sellPrice)))
				else
					exports['mythic_notify']:SendAlert('error', (Lang['not_your_company']))
				end
				...
				if(data.current.value == 'buy_certifcate') then
					ESX.TriggerServerCallback('t1ger_deliveryjob:buyCertifcate', function(certificateStatus)
						...
						elseif certificateStatus == 1 then 
							exports['mythic_notify']:SendAlert('inform', (Lang['alrdy_has_certificate']))
						elseif certificateStatus == true then 
							exports['mythic_notify']:SendAlert('success', (Lang['certificate_acquired']))
			...
			function(data, menu)
				if level >= data.current.level then 
					...
					elseif data.current.certificate == true and certifcate == 1 then 
						...
						exports['mythic_notify']:SendAlert('inform', (Lang['job_needs_certificate']))
					end
				else
					exports['mythic_notify']:SendAlert('error', (Lang['job_level_mismatch']))
				end
			end, function(data, menu)
				menu.close()
				ManageCompanyMenu(id,val)
...
function ShopOrderMenu(id, val)
	ESX.TriggerServerCallback('t1ger_deliveryjob:fetchShopOrders', function(orders)
		...
		if #orders > 0 then
			...
			exports['mythic_notify']:SendAlert('error', (Lang['no_available_orders']))
		end
	end)
end
	...
	function(data, menu)
		ESX.TriggerServerCallback('t1ger_deliveryjob:vehDepositFee', function(deposited)
			if deposited then
				...
				exports['mythic_notify']:SendAlert('success', (Lang['deposit_veh_paid']:format(data.current.deposit)))
				menu.close()
				...
				exports['mythic_notify']:SendAlert('error', (Lang['not_enough_to_deposit']))
			end
					...
					if IsControlJustPressed(0, Config.KeyToLoadJobVehicle) then
						...
						if curVeh > 0 then 
							if GetEntityModel(curVeh) == GetEntityModel(jobForklift) then
								...
								exports['mythic_notify']:SendAlert('error', (Lang['forklift_mismatch']))
							end
						else
							exports['mythic_notify']:SendAlert('error', (Lang['not_inside_forklift']))
						end
					end
				end
			end
		...
		if traillerFilledUp and not truckingStarted then 
			...
			if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 5.0) then 
				...
				if IsControlJustPressed(0, 47) then
					...
					DoScreenFadeIn(1000)
					Citizen.Wait(100)
					exports['mythic_notify']:SendAlert('inform', (Lang['trailer_filled_up']))
					SetTruckingRoute()
				end
			end
		end
						...
						if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, dSpot[1], dSpot[2], dSpot[3], true) < 4.0) then
							...
							if IsControlJustPressed(0, 38) then
								if not IsEntityInAir(palletObjEntity) then
									...
									exports['mythic_notify']:SendAlert('success', (Lang['park_fork_in_trailer']))
								else
									exports['mythic_notify']:SendAlert('success', (Lang['place_pallet_on_ground']))
								end
							end
						end
					end
						...
						if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 5.0) and DoesEntityExist(jobForklift) then 
							...
							if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 3.5) then 
								if IsControlJustPressed(0, 47) then
									...
									if deliveredPallets < maxPallets then 
										SetTruckingRoute()
										exports['mythic_notify']:SendAlert('success', (Lang['set_delivery_route']))
									elseif deliveredPallets >= maxPallets then 
										if DoesBlipExist(truckingBlip) then RemoveBlip(truckingBlip) end
										exports['mythic_notify']:SendAlert('success', (Lang['delivery_pallets_done']))
						...
						if IsControlJustPressed(0, Config.KeyToReturnJobVehicle) then
							...
							if curVeh > 0 then
								if GetEntityModel(curVeh) == GetEntityModel(jobVehicle) then
									...
									exports['mythic_notify']:SendAlert('error', (Lang['job_veh_mismatch']))
								end
							else
								exports['mythic_notify']:SendAlert('inform', (Lang['sit_in_job_veh']))
							end
						end
					end
				end
...
function PalletDeliveryPay()
	...
	if newVehBody < (truckHealth*dmgPercent) then 
		exports['mythic_notify']:SendAlert('error', (Lang['pallet_damaged_transit']))
		paycheck = paycheck
	else
		paycheck = paycheck + palletPrice
		exports['mythic_notify']:SendAlert('success', (Lang['paycheck_add_amount']:format(palletPrice)))
	end
end
								...
								if deliveredParcels < maxDeliveries then
									if IsEntityAttachedToAnyPed(deliveryParcel) then 
										...
										elseif deliveredParcels == maxDeliveries then 
											...
											exports['mythic_notify']:SendAlert('success', (Lang['delivery_complete']))
											if jobValue == 4 then
										...
										exports['mythic_notify']:SendAlert('error', (Lang['parcel_not_ind_hand']))
									end
								end
							end
						end
...
function ReturnVehAndGetPaycheck()
	...
	if newVehBody < (1000*dmgDeposit) then 
		giveDeposit = false
		exports['mythic_notify']:SendAlert('error', (Lang['deposit_not_returned']))
	else
		giveDeposit = true
	end
...
function ParcelDeliveryPay()
	...
	if newVehBody < (vehBodyHealth*dmgPercent) then 
		exports['mythic_notify']:SendAlert('error', (Lang['parcel_damaged_transit']))
		paycheck = paycheck
	else
		paycheck = paycheck + parcelPrice
		exports['mythic_notify']:SendAlert('success', (Lang['paycheck_add_amount']:format(parcelPrice)))
	end
end
			...
			if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 2.0) then 
				...
				if IsControlJustPressed(0, Config.KeyToPutParcelInVeh) then
					...
					if totalObjects == 0 then 
						...
						exports['mythic_notify']:SendAlert('success', (Lang['vehicle_filled_up']))
						fillingVeh = false
					end
				end
			end
...
function SpawnJobVehicle(vehModel, vehCoords, vehHeading)
	...
	exports['mythic_notify']:SendAlert('success', (Lang['job_veh_spawned']), 5000)
end
...
function SpawnTruckTrailer(trailerModel, vehCoords, vehHeading)
	...
	exports['mythic_notify']:SendAlert('success', (Lang['job_veh_spawned']))
end
