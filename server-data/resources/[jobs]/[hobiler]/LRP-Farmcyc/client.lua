    ...
    while true do
        Citizen.Wait(0)
			...
			for k in pairs(locations) do
				...
				ESX.ShowHelpNotification('Yengeç karides yakalamak için ~INPUT_PICKUP~ tuşuna basın')
			...
			if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						DrawText3Ds(vector3(Config.Processing.x,Config.Processing.y,Config.Processing.z), "Yengeçleri paketlemek için [E] tuşuna basın ", 0.35)
...
------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						---Draw3DText( Config.Processing.x, Config.Processing.y, Config.Processing.z , "~w~Produccion de cangrejos~y~\nPresiona [~b~E~y~] para comenzar el empaquetado",4,0.15,0.1)
						DrawText3Ds(vector3(Config.camarones.x,Config.camarones.y,Config.camarones.z), "Karidesleri paketlemek için [E] tuşuna basın", 0.35)
...
exports['np-taskbar']:taskBar(3500, "Aranıyor")
Citizen.Wait(250)
...
local making = true
	...
	exports['mythic_notify']:SendAlert('inform', 'Yengeç üretiliyor', 2500)
	Citizen.Wait(5000)
...
local makingcamarones = true
	...
	exports['mythic_notify']:SendAlert('inform', 'Karides üretiliyor', 2500)
	Citizen.Wait(5000)
...
Citizen.Wait(1)
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.ventacyc.x, Config.ventacyc.y, Config.ventacyc.z, true) < 2 then
   	DrawText3Ds(vector3(Config.ventacyc.x,Config.ventacyc.y,Config.ventacyc.z), "Magaza menüsünü açmak için [E] tuşuna basın", 0.35)
...
local elements ={  
{ label = 'Karides Satısı <span style = "color:#65D754;">' ..Config.PCamaron ..  '$</span>', value = 'Camarones1'},

{ label = 'Yengec Satisi <span style = "color:#65D754;">' ..Config.PCangrejo ..  '$</span>', value = 'Cangrejos1'},

{ label = 'Satış yerinden <span style = "color:#DA2020;">ayrıl</span>', value = 'salir'},

}

ESX.UI.Menu.CloseAll() 
ESX.UI.Menu.Open('default', GetCurrentResourceName(),'ventacyc',{
	title = 'Balıkçı Restaurant', 
	align = 'top-left',
	elements = elements  
