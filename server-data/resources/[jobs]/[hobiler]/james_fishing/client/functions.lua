...
TryToFish = function()
    if IsPedSwimming(cachedData["ped"]) then return exports["mythic_notify"]:SendAlert("error", "Yüzerken balık tutamazsın!") end
    if IsPedInAnyVehicle(cachedData["ped"]) then return exports["mythic_notify"]:SendAlert("error", "Balık tutmak için araçtan inmelisin!") end

    if not HasItems({Config.FishingItems["rod"]["name"]}) then return exports["mythic_notify"]:SendAlert("error", "Balık tutmak için oltaya ihtiyacın var.") end    
    if not HasItems({Config.FishingItems["bait"]["name"]}) then return exports["mythic_notify"]:SendAlert("error", "Balık tutmak için yeme ihtiyacın var.") end
    ...
    if waterValidated then
        ...
        exports["mythic_notify"]:SendAlert("error", "Balık tutmak için denize bakmalısın.")
    end
end
...
CastBait = function(rodHandle, castLocation)
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            exports["mythic_notify"]:SendAlert("error", "Önce yemi takmalısın!")
    ...
    if caughtFish then
        ESX.TriggerServerCallback("james_fishing:receiveFish", function(received)
            if received then
                exports["mythic_notify"]:SendAlert("success", "Balık yakaladın!")
            end
        end)
    else
        exports["mythic_notify"]:SendAlert("error", "Balık kaçtı!")
    end
    CastBait(rodHandle, castLocation)
end
...
SellFish = function()
    if not HasItems({
        Config.FishingItems["fish"]["name"]
            ...
            exports["mythic_notify"]:SendAlert("error", "Hiç balığın yok") end
    ...
    ESX.TriggerServerCallback("james_fishing:sellFish", function(sold, fishesSold)
        if sold then
            exports["mythic_notify"]:SendAlert("inform", sold.. '$ karşılığında ' .. fishesSold .. ' tane balık sattın ')

            -- exports.pNotify:SendNotification({
            --     text = (sold.. '$ karşılığında ' .. fishesSold .. ' tane balık sattın.'),
            --     type = "info",
            --     timeout = 2000,
            --     layout = "topRight",
            --     queue = "right"
            ...
            exports["mythic_notify"]:SendAlert("error", "Yarın bidaha gel !")
