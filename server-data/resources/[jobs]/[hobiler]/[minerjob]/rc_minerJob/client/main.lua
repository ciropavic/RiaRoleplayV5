...
RegisterCommand('madencimenukapat', function(source, args, rawCommand)
	SetDisplay(false)
	Citizen.Wait(3000)
	isMenuOn = false
end)
...
local blips = {
	{title="Maden Ocağı", colour=0, id=357, x=-593.95855712891, y=2091.75390625, z=131.71856689453},
  }
...
AddEventHandler("showReward:minerJob", function(completReward)
	exports['mythic_notify']:SendAlert('success', completReward..' $ kazandınız')
end)
		...
		if delivery_point_distance < 500.0 then
			...
			if delivery_point_distance < 1.5 then
				if isMining == false then
					DisplayHelpText("Kazmak icin ~INPUT_CONTEXT~ tusuna bas")
					...
					if(IsControlJustReleased(1, 38))then
						if pickaxeCounter > 0 then
							...
							exports['mythic_notify']:SendAlert('error', 'İş için kazma gerekli', 2500)
						end
...
AddEventHandler("pickaxeBroken:minerJob", function()
	exports['mythic_notify']:SendAlert('error', 'Kazma Kırıldı', 2500)
end)
