...
AddEventHandler('TireEntendu', function(gx, gy, gz)
     if PlayerData.job ~= nil and PlayerData.job.name == 'justice' then
          --PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 0)
          ESX.ShowAdvancedNotification(
               'LSPD ALARMI', 
               'MERKEZ LSPD', 
               'Silah sesleri hakkında çağrıyı ~g~Y~s~  çağrıyı reddetmek için  ~r~X~s~ e basınız..', 'CHAR_CHAT_CALL', 8)
          coordsX = gx
          coordsY = gy
          coordsZ = gz
          Citizen.Wait(1000)
