...
function OpenCloakroomMenu()
	...
	if grade == 'recruit' then
		table.insert(elements, {label = _U('justice_wear'), value = 'recruit_wear'})
	elseif grade == 'officer' or grade == 'sous-officier' or grade == 'Off-stup' then
		table.insert(elements, {label = _U('justice_wear'), value = 'officer_wear'})
	elseif grade == 'sergeant' or grade == 'sergent-stup' then
		table.insert(elements, {label = _U('justice_wear'), value = 'sergeant_wear'})
	elseif grade == 'lieutenant2' then
		table.insert(elements, {label = _U('justice_wear'), value = 'lieutenant2_wear'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = _U('justice_wear'), value = 'lieutenant_wear'})
	elseif grade == 'lieutenant3' then
		table.insert(elements, {label = _U('justice_wear'), value = 'lieutenant3_wear'})
	elseif grade == 'lieutenant4' then
		table.insert(elements, {label = _U('justice_wear'), value = 'lieutenant4_wear'})
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('justice_wear'), value = 'boss_wear'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'justice')
						TriggerServerEvent('esx_service:disableService', 'justice')
						TriggerEvent('esx_justicejob:updateBlip')
		...
		if
			...
			data.current.value == 'lieutenant3wear' or
			data.current.value == 'lieutenant4wear' or
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_justicejob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_justicejob:buyJobVehicle', function (bought)
					if bought then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'justice_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = '<span style="color:orange;">Durum <span style="color:cyan;"> >', value = 'status'},
		--	{label = '<span style="color:#00cc00;">Vatandaş Etkileşimi<span style="color:cyan;"> >', value = 'citizen_interaction'},
		--	{label = '<span style="color:#00cc00;">Araç Etkileşimi <span style="color:cyan;"> >', value = 'vehicle_interaction'},
			{label = '<span style="color:#669999;">Kelepçe<span style="color:cyan;"> >', value = 'menu_menottes'},			
		--	{label = '<span style="color:#669999;">Objeler<span style="color:cyan;"> >', value = 'object_spawner'},
		--	{label = '<span style="color:#ff0033;">Güvenlik Menüsü<span style="color:cyan;"> >', value = 'securitymenu'},
			{label = '<span style="color:#ff0033;">Para Cezası<span style="color:cyan;"> >', value = 'ammende'},
			{label = '<span style="color:#ff0033;">Hapis<span style="color:cyan;"> >', value = 'jail_menu'},
		--	{label = '<span style="color:#006eff;">Takviye Talebi<span style="color:cyan;"> >', value = 'renfort'},
			
	}}, function(data, menu)
	
		if data.current.value == 'rd_mobile' then
            TriggerEvent('justice:justice_radar')
		end
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job and PlayerData.job.name == 'justice' and PlayerData.job.grade_name == 'sergeant' or PlayerData.job.grade_name == 'lieutenant' or PlayerData.job.grade_name == 'boss' then
				table.insert(elements, {label = 'Ruhsatları geri ver!', value = 'ppa'})
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_justicejob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'ppa' then
						local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
						if closestPlayer ~= -1 and closestDistance <= 3.0 then
						exports['mythic_notify']:SendAlert('inform',_U('licence_you_ppa', data.current.label, targetName))
					...
					elseif action == 'handcuff' then
						--TriggerServerEvent('esx_justicejob:handcuff', GetPlayerServerId(closestPlayer))
						...
						if distance <= 2.0 then
							TriggerServerEvent('esx_justicejob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
							Wait(5000)
							TriggerServerEvent('esx_justicejob:handcuff', GetPlayerServerId(closestPlayer))
						else
							exports['mythic_notify']:SendAlert('error','Oyuncu yeterince Yakın Değil')
						end
					elseif action == 'handcuff2' then
						...
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						local target_id = GetPlayerServerId(target)
						TriggerServerEvent('esx_justicejob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						Wait(5000)
						TriggerServerEvent('esx_justicejob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_justicejob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_justicejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_justicejob:OutVehicle', GetPlayerServerId(closestPlayer))
					...
					if amount == nil or amount <= 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							Citizen.CreateThread(function()
								...
								TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_justice', 'Adalet', amount)
								exports['mythic_notify']:SendAlert('error','Para cezası kesildi')							
		...
		elseif data.current.value == 'menu_menottes' then
			...
			local elements = {
				{label = '<span style="color:green;">Kelepçele<span style="color:white;">', value = 'handcuff'},
				{label = '<span style="color:red;">Kelepçeyi çıkar<span style="color:white;">', value = 'handcuff2'},
				{label = 'Kişiye eşlik edin', value = 'drag'},
				{label = '<span style="color:green;"><span style="color:white;"> Araca Koy', value = 'put_in_vehicle'},
				{label = '<span style="color:red;"><span style="color:white;"> Araçtan Çıkar', value = 'out_the_vehicle'},
		--		{label = '<span style="color:#006eff;">Barut Testi<span style="color:cyan;"> >', value = 'gsr_test'}
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if action == 'handcuff' then
						...
						if distance <= 2.0 then
							...
							exports['mythic_notify']:SendAlert('error','Yeterince Yakın Değil')
						end
		...
		elseif data.current.value == 'securitymenu' then	
			...
			local elements = {
				{label = '👮 Eskort Talep Et (1 tane)', value = 'spawn'},	
				{label = '👮 Eskort Talep Et  (2 tane)', value = 'spawn2'},  
				{label = '🗡️ En yakın oyuncuya saldır', value = 'playerattack'},
				{label = '🚘 Araç: Fix', value = 'fix'},
				{label = '🛫 Çekip git!', value = 'wander'},
				{label = '🔊 Beni takip et', value = 'follow'},
				{label = '🗑️ Kaldır', value = 'delete'},
			}
			
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'securitymenu', {
                css      = 'justice',
				title    = 'Menu Polis Destek',
				align    = 'top-left',
				elements = elements
			}, function(data2, menu2)
				local action = data2.current.value
		...
		elseif data.current.value == 'status' then
			...
			local elements = {
				{label = '<span style="color:green;">Memur<span style="color:white;"> Göreve Başla', value = 'prise'},
				{label = '<span style="color:red;">Memur<span style="color:white;"> Görevi Bitir', value = 'fin'},
				{label = '<span style="color:orange;">Memur<span style="color:white;">Görevi Durdur', value = 'pause'},
				{label = '<span style="color:orange;">Adli Dava<span style="color:gray;">Adli davadayım', value = 'standby'},
				{label = '<span style="color:orange;">Müvekkil Bekleme<span style="color:gray;"> Müvekkil savunması', value = 'control'},
				{label = '<span style="color:orange;">Geliyorum<span style="color:gray;"> Geliyorum yoldayım', value = 'refus'},
				{label = '<span style="color:orange;">Dava Var<span style="color:gray;"> Dava var acil toplanmanız gerekiyor', value = 'crime'}
			}

			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'status_service', {
				css      = 'justice',
				title    = 'Adliye Durum Servisi',
				align    = 'top-left',
				elements = elements
			}, function(data2, menu2)
				local action = data2.current.value
				...
				if action == 'prise' then
					local info = 'prise'
					TriggerServerEvent('justice:PriseEtFinservice', info)
		...
		elseif data.current.value == 'renfort' then
			...
			table.insert(elements, {label = ('Tekli Ekip İsteği'), value = 'petite_demande'})
			table.insert(elements, {label = ('İkili Ekip İsteği'), value = 'demande_importante'})
			table.insert(elements, {label = ('Tüm birimler talep edildi!'), value = 'omgad'})


			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'renfort', {
				css      = 'justice',
				title    = ('Menu Meslek Destek'),
				align    = 'top-left',
				elements = elements
			}, function(data2, menu2)
				local coords  = GetEntityCoords(playerPed)
		...
		elseif data.current.value == 'lspd_equipement' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lspd_equipement', {
				...
				if action == 'equipement_cadet' then
					...
					ESX.ShowAdvancedNotification('Adliye INFO', 'Armurie Adliye', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
				...
				elseif action == 'equipement_sergeant' then
					...
					ESX.ShowAdvancedNotification('Adliye INFO', 'Armurie Adliye', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
					PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
				elseif action == 'equipement_lieutenant2' then
					PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
					Citizen.Wait(2000)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_COMBATPISTOL", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_STUNGUN", 255, false, false)
				...
				elseif action == 'equipement_lieutenant' then
					...
					ESX.ShowAdvancedNotification('Adliye INFO', 'Armurie Adliye', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
					PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
				elseif action == 'equipement_lieutenant3' then
					PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
					Citizen.Wait(2000)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_COMBATPISTOL", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_STUNGUN", 255, false, false)
					...
					ESX.ShowAdvancedNotification('Adliye INFO', 'Armurie Adliye', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
					PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
				elseif action == 'equipement_lieutenant4' then
					PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
					Citizen.Wait(2000)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_COMBATPISTOL", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_STUNGUN", 255, false, false)
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_justicejob:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('esx_justicejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_justicejob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_justicejob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_justice', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_justicejob:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_justicejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			exports['mythic_notify']:SendAlert('success',_U('licence_you_revoked', data.current.label, targetName))
			TriggerServerEvent('esx_justicejob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_justicejob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('esx_justicejob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_justicejob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
--			end
--		else
--			ESX.TriggerServerCallback('esx_justicejob:buyWeapon', function(bought)
--				if bought then
--					if data.current.price > 0 then
--						exports['mythic_notify']:SendAlert('error',_U('armory_bought', data.current.weaponLabel, ESX.Math.GroupDigits(data.current.price)))
--					end
--
--					menu.close()
--					OpenBuyWeaponsMenu()
...
function OpenWeaponComponentShop(components, weaponName, parentShop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons_components', {
		...
		if data.current.hasComponent then
			exports['mythic_notify']:SendAlert('success',_U('armory_hascomponent'))
		else
			ESX.TriggerServerCallback('esx_justicejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
					...
					exports['mythic_notify']:SendAlert('success',_U('armory_money'))
				end
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_justicejob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					exports['mythic_notify']:SendAlert('success',_U('quantity_invalid'))
				else
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_justicejob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_justicejob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_justicejob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	PlayerData.job = job
	Citizen.Wait(5000)
	TriggerServerEvent('esx_justicejob:forceBlip')
...
AddEventHandler('esx_addons_gcphone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'justice' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_justicejob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_justicejob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_justicejob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job and PlayerData.job.name == 'justice' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_justicejob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_justicejob:handcuff')
AddEventHandler('esx_justicejob:handcuff', function()
...
RegisterNetEvent('esx_justicejob:unrestrain')
AddEventHandler('esx_justicejob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_justicejob:drag')
AddEventHandler('esx_justicejob:drag', function(copId)
...
RegisterNetEvent('esx_justicejob:putInVehicle')
AddEventHandler('esx_justicejob:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_justicejob:OutVehicle')
AddEventHandler('esx_justicejob:OutVehicle', function()
	local playerPed = PlayerPedId()
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'justice' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_justicejob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_justicejob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_justicejob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_justicejob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'justice' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'justice' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'justice_actions') then
			if Config.MaxInService == -1 then
...
AddEventHandler('renfort:setBlip', function(coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Takviye talebi', 'Takviye talebi talep edildi.\nKod: ~g~CODE-2\n~w~Importance: ~g~Yardım lazım.', 'CHAR_CALL911', 8)
		Wait(1000)
	...
	elseif raison == 'importante' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		PlaySoundFrontend(-1, "OOB_Start", "GTAO_FM_Events_Soundset", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Takviye talebi', 'Takviye talebi talep edildi.\nKod: ~g~CODE-3\n~w~Importance: ~o~Acil değil ama yardım gerçekten lazım', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
		color = 47
	elseif raison == 'omgad' then
		...
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Takviye talebi', 'Takviye talebi talep edildi.\nKod: ~g~CODE-99\n~w~Importance: ~r~ACİL YARDIM GEREK!!!', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
...
RegisterNetEvent('justice:InfoService')
AddEventHandler('justice:InfoService', function(service, nom)
	if service == 'prise' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Mesai Durumu', 'Memur: ~g~'..nom..'\n~w~Kod: ~g~10-8\n~w~Bilgi: ~g~Mesaiye başladım', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'fin' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Mesai Durumu', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-10\n~w~Bilgi: ~r~Mesaiyi bitirmek üzereyim', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'pause' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Mesai Bitiş', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-6\n~w~Bilgi: ~r~Mesai bitti bay bay enayiler', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'standby' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Adli Dava', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-12\n~w~Bilgi:~r~ Adliyedeyim davadayım', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'control' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~bMüvekkil Bekleme ', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-48\n~w~Bilgi: ~r~Bir müvekkili savunuyorum müsait değilim', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'refus' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~r~Geliyorum', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-30\n~w~Bilgi: ~r~Adliyeye geliyorum', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'crime' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Adliye DURUM', '~b~Dava Var!', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-31\n~w~Bilgi: ~r~Adliyeye gelin dava başlıyor!', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	end
end)
...
RegisterNetEvent('esx_justicejob:updateBlip')
AddEventHandler('esx_justicejob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'justice' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'justice' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_justicejob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_justicejob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_justicejob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'justice')
...
RegisterNetEvent('esx_justicejob:getarrested')
AddEventHandler('esx_justicejob:getarrested', function(playerheading, playercoords, playerlocation)
...
RegisterNetEvent('esx_justicejob:doarrested')
AddEventHandler('esx_justicejob:doarrested', function()
	Citizen.Wait(250)
	loadanimdict('mp_arrest_paired')
...
RegisterNetEvent('esx_justicejob:douncuffing')
AddEventHandler('esx_justicejob:douncuffing', function()
	Citizen.Wait(250)
	loadanimdict('mp_arresting')
...
RegisterNetEvent('esx_justicejob:getuncuffed')
AddEventHandler('esx_justicejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	z = z - 1.0
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
