...
AddEventHandler('AnnounceEMSOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS Durum', '~g~Bir EMS hizmette! Önce sağlığınız!', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('AnnounceEMSFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS Durum', '~r~Artık hizmette EMS yok!', 'CHAR_CALL911', 8)
	end
end)
