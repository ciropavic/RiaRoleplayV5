...
function OpenCloakroomMenu()
	...
	elseif grade == 'lieutenant3' then
		table.insert(elements, {label = _U('sheriff_wear'), value = 'lieutenant3_wear'})
	elseif grade == 'lieutenant4' then
		table.insert(elements, {label = _U('sheriff_wear'), value = 'lieutenant4_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			...
			data.current.value == 'lieutenant3' or
			data.current.value == 'lieutenant4' or
...
RegisterNetEvent("sheriff:RecieveMenu") -- For opening the emote menu from another resource.
AddEventHandler("sheriff:RecieveMenu", function()
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sheriff_actions', {
		...
		elements = {
			...
			{label = '<span style="color:#006eff;">Takviye Talebi<span style="color:cyan;"> >', value = 'renfort2'},
			
	}}, function(data, menu)
	
		if data.current.value == 'rd_mobile' then
            TriggerEvent('sheriff:POLICE_radar')
		end
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job and PlayerData.job.name == 'sheriff' and PlayerData.job.grade_name == 'lieutenant3' or PlayerData.job.grade_name == 'lieutenant4'   or PlayerData.job.grade_name == 'boss' then
		...
		elseif data.current.value == 'status' then
			...
			local elements = {
				...
				{label = '<span style="color:orange;">Sevk<span style="color:gray;"> Sevk İçin Bekliyorum', value = 'standby'},
				{label = '<span style="color:orange;">Kontrol<span style="color:gray;"> Devam Eden Takip', value = 'control'},
				{label = '<span style="color:orange;">Uyum<span style="color:gray;"> Uyum reddi', value = 'refus'},
				{label = '<span style="color:orange;">Suçlu Takip<span style="color:gray;"> devam ediyor', value = 'crime'}
			}

			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'status_service', {
				css      = 'sheriff',
				title    = 'SHERIFF Durum Servisi',
				align    = 'top-left',
				elements = elements
			}, function(data2, menu2)
				local action = data2.current.value
		...
		elseif data.current.value == 'lspd_equipement' then
			...
			elseif grade == 'lieutenant2' then
				table.insert(elements, {label = ('Equipement Lieutenant'), value = 'equipement_lieutenant2'})
			elseif grade == 'lieutenant' then
				table.insert(elements, {label = ('Equipement Lieutenant'), value = 'equipement_lieutenant'})
			elseif grade == 'lieutenant3' then
				table.insert(elements, {label = ('Equipement Lieutenant'), value = 'equipement_lieutenant3'})
			elseif grade == 'lieutenant4' then
				table.insert(elements, {label = ('Equipement Lieutenant'), value = 'equipement_lieutenant4'})
			elseif grade == 'boss' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lspd_equipement', {
				...
				if action == 'equipement_cadet' then
					...
					ESX.ShowAdvancedNotification('SHERIFF INFO', 'Armurie SHERIFF', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
...
--	--SetBlipName(blipsRenfort, 'Demande de renfort2')
...
AddEventHandler('renfort2:setBlip', function(coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Takviye talebi', 'Takviye talebi talep edildi.\nKod: ~g~CODE-2\n~w~Importance: ~g~Yardım lazım.', 'CHAR_CALL911', 8)
		Wait(1000)
...
AddEventHandler('sheriff:InfoService', function(service, nom)
	if service == 'prise' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Mesai Durumu', 'Memur: ~g~'..nom..'\n~w~Kod: ~g~10-8\n~w~Bilgi: ~g~Mesaiye başladım', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	...
	elseif service == 'pause' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Mesai Bitiş', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-6\n~w~Bilgi: ~r~Mesai bitti bay bay enayiler', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'standby' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Sevk Durumu', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-12\n~w~Bilgi:~g~ Beklemedeyim, gönderilmeyi bekliyorum.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'control' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Trafik Kontrolü', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-48\n~w~Bilgi: ~g~Yol trafik kontrolüne devam ediyorum.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'refus' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~r~Uyum Reddi!', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-30\n~w~Bilgi: ~r~Uyum reddedildi. Suçlu peşindeyim.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'crime' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('SHERIFF DURUM', '~b~Devam Eden Suç', 'Memur: ~g~'..nom..'\n~w~Code: ~g~10-31\n~w~Bilgi: ~r~Suçlunun peşindeyim takip devam ediyor', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	end
end)
