...
ESX.RegisterServerCallback('esx_documents:deleteDocument', function(source, cb, id)
    ...
    MySQL.Async.execute('DELETE FROM user_documents WHERE id = @id AND owner = @owner',
    {
        ...
        if rowsChanged >= 1 then
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success',text = _U('document_deleted')})
            cb(true)
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success',text =  _U('document_delete_failed')})
            cb(false)
        end
        end)
end)
            ...
            if id ~= nil then
                MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', {['@id']=id}, function (result)
                    ...
                    if(result[1] ~= nil) then
                        ...
                        TriggerClientEvent('mythic_notify:client:SendAlert', _targetid, { type = 'success',text =  _U('copy_from_player')})
                        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success',text =  _U('from_copied_player')})
                    else
                        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success',text =  _U('could_not_copy_form_player')})
                    end
                end)
            else
                TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success',text =  _U('could_not_copy_form_player')})
