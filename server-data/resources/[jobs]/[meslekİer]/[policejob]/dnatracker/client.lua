  ...
  while true do
    ...
    if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
      ...
      if distA and distA < self.DrawTextDist then
        drawing = "ammo"
        self:DrawText3D(self.AmmoAnalyzePos.x,self.AmmoAnalyzePos.y,self.AmmoAnalyzePos.z, "Mermi analizi için [~g~E~s~] tusuna bas")
      elseif distB and distB < self.DrawTextDist then
        drawing = "blood"
        self:DrawText3D(self.DNAAnalyzePos.x,self.DNAAnalyzePos.y,self.DNAAnalyzePos.z, "Dna analizi için [~g~E~s~] tusuna bas")
      end
      if drawing then
...
function MFD:HandleDrawText(plyPed)
  ...
  if closestDist and closestDist < 5.0 then
    local str = ''
    if closest.obj == self.BloodObject then str = "[ ~r~DNA~s~ ] : İnsan Kanı: Örnek almak için [~r~ E ~s~] tusuna basınız" else str = "[ ~r~EVIDENCE ~s~] : Mermi Kovanı : Kovanı almak için [~r~ E ~s~] tusuna basınız."; end 
    ...
    if (IsControlJustPressed(1, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) and (GetGameTimer() - self.timer) > 150 then
      ...
      ESX.TriggerServerCallback('DnaTracker:PickupEvidenceS', function(canPickup)
        if not canPickup then ESX.ShowNotification("Bunlardan daha fazlasını alamazsınız.")
        else
...
AddEventHandler('DnaTracker:AnalyzeDNA', function()
  ...
  local elements = {
    [1] = { label = "İsim Soyisim: " .. self.CurBlood.name},
    [2] = { label = "Tip: Kan" },
    [3] = { label = "Bulunduğu Cadde: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Konum: [ X : " .. math.floor(self.CurBlood.pos.x) .. " ] [ Y : " .. math.floor(self.CurBlood.pos.y) .. "] : [ Z : " .. math.floor(self.CurBlood.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Blood_Info", { title = "Dna Analiz Makinası", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
...
AddEventHandler('DnaTracker:AnalyzeAmmo', function()
  ...
  local elements = {
    [1] = { label = "Silah: " .. newStr},
    [2] = { label = "Mermi Tipi: " .. self.CurBullet.weaType },
    [3] = { label = "Bulunduğu Cadde: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Konum : [ X : " .. math.floor(self.CurBullet.pos.x) .. " ] : [ Y : " .. math.floor(self.CurBullet.pos.y) .. "] : [ Z : " .. math.floor(self.CurBullet.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "AnalyzeAmmo", { title = "Mermi Analiz Makinası", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
      ESX.UI.Menu.CloseAll()          
    end,    
