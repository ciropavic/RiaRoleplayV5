...
TriggerEvent('es:addCommand', 'gsr', function(source, args, user)
    ...
    if args[1] ~= nil then 
		if xPlayer.job.name == 'police' and type(number) == "number" then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Test etmek için polis olmalısın' })
    	end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Doğru kullanım /gsr userid' })
	end
end)
...
function gsrcheck(source, identifier)
	...
	if Config.UseCharName then
		...
		if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Barut testi '..fullName..' pozitif geldi.', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'Barut testi '..fullName..' negatif geldi.', length = 5000 })
    	end
	else
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Barut testi pozitif geldi', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'Barut testi negatif geldi', length = 5000 })
    	end
	end
end
...
AddEventHandler('GSR:Status2', function(playerid)
    ...
    if Config.UseCharName then
    	...
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Barut testi poziif geldi', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'Barut testi negatif geldi', length = 5000 })
    	end
	end
