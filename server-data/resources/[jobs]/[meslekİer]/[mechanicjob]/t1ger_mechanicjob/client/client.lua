	...
	function(data, menu)
		if(data.current.value == 'rename_mech_shop') then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_rename', {
				title = "Mekaniği Yeniden Adlandır"
	...
	function(data, menu)
		...
		if(data.current.value == 'job_grade_manage') then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_update_new_job_grade', {
				title ="Mevcut Iş Derecesi: "..user.jobGrade
			}, function(data2, menu2)
				menu2.close()
...
function SellMechShopMenu(id,val)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_sell_menu",
		{
			title    = "Satışı Onayla | Fiyat:: $"..math.floor(sellPrice),
			align    = "center",
			elements = elements
		},
	function(data, menu)
...
function BuyMechShopMenu(id,val)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_purchase",
		{
			title    = "Satışı Onayla | Fiyat:"..math.floor(val.price),
			align    = "center",
			elements = elements
		},
	function(data, menu)
	...
	function(data, menu)
		if(data.current.value == 'storage_deposit') then
			ESX.TriggerServerCallback('t1ger_mechanicjob:getUserInventory', function(inventory)
				...
				function(data2, menu2)
					menu2.close()
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'storage_deposit_dialog', {title = "Miktarı girin"}, function(data3, menu3)
		...
		if(data.current.value == 'storage_withdraw') then
			ESX.TriggerServerCallback('t1ger_mechanicjob:getStorageInventory', function(inventory)
				if inventory ~= nil then 
					...
					function(data2, menu2)
						menu2.close()
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'storage_withdraw_dialog', {title = "Miktarı girin"}, function(data3, menu3)
							local count = tonumber(data3.value)
...
function MechShopWorkbenchMenu(id,val)
	...
	function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "mech_workbench_options_menu",
			{
				title    = "Imalat Edilebilir: "..data.current.label,
				align    = "center",
		...
		function(data2, menu2)
			if data2.current.value == "craft_item" then 
				...
				if not keyPressed then 
					keyPressed = true
					exports['taskbar']:taskBar((Config.CraftTime * 1000), (Lang['crafting_item']:format(string.upper(selected.label))))
...
function OpenMechanicActionMenu()
	if PlayerData.job and PlayerData.job.name == "mechanic" then
		local elements = {
			{ label = "Faturalandırma", value = "mech_billing" },
			{ label = "Kriko Kullan", value = "use_car_jack" },
			{ label = "Araç Incele", value = "inspect_vehicle" },
			{ label = "Araç Parçalarını Onar", value = "repair_health_parts" },
			{ label = "Araç Motor Onarımı", value = "veh_engine_repair" },
			{ label = "Araç Gövde Onarımı", value = "veh_body_repair" },
		--	{ label = "Prop Emoteları", value = "prop_emotes" },
			{ label = "NPC Araçları", value = "npc_jobs" },
		}
				...
				while true do 
					...
					if distance < 5.0 then
						...
						if IsControlJustPressed(0, 38) and distance < 1.0 then
							...
							exports['taskbar']:taskBar(2000, "INSPECTING: ENGINE")
							Citizen.Wait(2000)
							...
							if engineValue < 10.0 then 
								...
								ESX.TriggerServerCallback('t1ger_mechanicjob:getMaterialsForHealthRep', function(hasMaterials)
									if hasMaterials then 
										...
										exports['taskbar']:taskBar(3500, "REPAIRING: ENGINE")
										Citizen.Wait(3500)
								...
								ShowNotifyESX("Motor tamamen işlevsel")
							end
							break
						end
					end
		...
		while true do 
			...
			if distance < 5.0 then
				...
				if IsControlJustPressed(0, 38) and distance < 1.0 then
					...
					Citizen.Wait(500)
					TaskStartScenarioInPlace(player, "WORLD_HUMAN_VEHICLE_MECHANIC", 0, true)
					exports['taskbar']:taskBar(3500, (Lang['lift_repairing_veh']:format(string.upper(label))))
					Citizen.Wait(3500)
					ClearPedTasks(player)
					ESX.TriggerServerCallback('t1ger_mechanicjob:getMaterialsForHealthRep', function(hasMaterials)
						if hasMaterials then 
							...
							if distance < 4.0 then
								if not v.done then 
									...
									if IsControlJustPressed(0, 38) and distance < 1.0 then
										...
										exports['taskbar']:taskBar(1500, Lang['progbar_inspecting_veh'])
										Citizen.Wait(1500)
...
function UseTheJackFunction(vehicle)
	...
	exports['taskbar']:taskBar((6500), label)
	-- Raise Jack Task:
			...
			if distance < 4.0 then
				if not v.done then 
					...
					if IsControlJustPressed(0, 38) and distance < 1.0 then
						...
						TaskStartScenarioInPlace(player, v.scenario, 0, true)
						exports['taskbar']:taskBar(2500, Lang['progbar_analyzing_veh'])
						Citizen.Wait(2500)
						ClearPedTasks(player)
						v.done = true
					end
...
function RepairVehOptions(veh, type, val)
	...
	exports['taskbar']:taskBar((repairDuration), val.progbar)
...
function ManageNpcJobs()
	local elements = {
		{ label = "Iş Ara", value = "find_job" },
		{ label = "Işi Iptal Et", value = "cancel_job" },
	}
	...
	function(data, menu)
		if(data.current.value == 'find_job') then
			...
			if count == 100 then
				ShowNotifyESX("Telefon İçin Bekle")
			else
			...
			if distance < 10.0 then
					...
					if IsControlJustPressed(0, 38) and vehDistance < 1.0 and not buttonClicked then
						...
						ESX.TriggerServerCallback('t1ger_mechanicjob:getInventoryItem', function(hasItem)
							if hasItem then 
								...
								exports['taskbar']:taskBar((4000), Lang['progbar_npc_fix'])
								Citizen.Wait(4000)
				...
				if pedDistance < 5.0 and vehicleRepaired then
					...
					if IsControlJustPressed(0, 38) and pedDistance < 1.5 then
						...
						exports['taskbar']:taskBar((2000), Lang['progbar_npc_cash'])
						Citizen.Wait(2000)
						ClearPedTasks(GetPlayerPed(-1))
