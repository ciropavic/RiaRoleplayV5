	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='success', text = _U('have_withdrawn', count, item.name)})
			else
				TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text= _U('player_cannot_hold')})
			end
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text= _U('not_enough_in_society')})
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if item.count >= 0 then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='success', text =_U('have_deposited', count, item.name)})
		else
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)', {
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='success', text = _U('vehicle_belongs',plate)})
			cb(true)
		end)
	else
		cb(false)
			...
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					if xPlayer.getAccount('bank').money >= v.rent_price then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source , {type = 'success', text = _U('paid_rental', ESX.Math.GroupDigits(v.rent_price), v.plate)})
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source , {type = 'success', text =_U('paid_rental_evicted', ESX.Math.GroupDigits(v.rent_price), v.plate)})
