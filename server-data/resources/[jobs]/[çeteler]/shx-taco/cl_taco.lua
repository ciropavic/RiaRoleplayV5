...
function TeslimatBasla()
    ...
    exports['mythic_notify']:SendAlert('success', 'Green taco dağıtımı başladın!')
...
function hangitacoyapsash1no()
	...
	menu.close()
	exports['mythic_notify']:SendAlert('error','Yapmaktan vazgeçtin!')
            ...
            if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3)  then
				if not inMission then
					...
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						...
						inMission = false
						exports['mythic_notify']:SendAlert('error','Green taco dağıtımı iptal ettin!')
					    end
				    end
                end
            end
