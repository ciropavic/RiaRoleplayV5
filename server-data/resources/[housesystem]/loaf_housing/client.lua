...
AddEventHandler('loaf_housing:setHouse', function(house, purchasedHouses)
    ...
    for k, v in pairs(Config.Houses) do
        if OwnedHouse.houseId == k then
            blipolustur(v['door'], 374, 3, 0.3, Strings['Your_House'])
        else
            if not AvailableHouses[k] then
                if Config.AddHouseBlips then
                    blipolustur(v['door'], 374, 0, 0.3, '')
                end
            else
                if Config.AddBoughtHouses then
                    blipolustur(v['door'], 374, 2, 0.5, Strings['Player_House'])
                end
            end
        end
    end
...
blips = {}
RegisterNetEvent('mak:togglehouseblip')
AddEventHandler('mak:togglehouseblip',function()
    if not bliptrue then
        bliptrue = true
        for k, v in pairs(Config.Houses) do
            blipolustur(v['door'], 374, 0, 0.3, '')-- 75 = Blip 4 = color 0.6 = blip scale oluyor
            ESX.ShowNotification('Evlerin blipi açıldı')
        end
        ESX.ShowNotification('Evlerin blipi açıldı')
    else
        bliptrue = false
        ...
        ESX.ShowNotification('Evlerin blipleri kapatıldı')
    end
end)
