		...
		elseif data.current.value == 'attached_animal' then
			if not IsPedSittingInAnyVehicle(ped) then
				if isAttached == false then
					...
					exports['mythic_notify']:SendAlert('error',_U('dontattachhiminacar'))
			end
			...
			if distance < 5 then
				if count >= 1 then
					if status < 100 then
						...
						exports['mythic_notify']:SendAlert('success',_U('gavepetfood'))
						...
						exports['mythic_notify']:SendAlert('success', _U('nomorehunger'))
					end
				else
					exports['mythic_notify']:SendAlert('error',_U('donthavefood'))
				end
			else
				exports['mythic_notify']:SendAlert('error',_U('hestoofar'))
			end
			...
			if not isInVehicle then
				if IsPedSittingInAnyVehicle(playerPed) then
					if distance < 8 then
						...
						exports['mythic_notify']:SendAlert('error',_U('toofarfromcar'))
					end

				else
					exports['mythic_notify']:SendAlert('error', _U('youneedtobeincar'))
				end
			else
				if not IsPedSittingInAnyVehicle(playerPed) then
					...
					exports['mythic_notify']:SendAlert('error', _U('yourstillinacar'))
				end
			end
		end
	...
	ESX.TriggerServerCallback('eden_animal:getPet', function(pet)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_orders', {
			...
			if data.current.value == 'return_doghouse' then
				...
				exports['mythic_notify']:SendAlert('success',_U('doghouse_returning'))
	...
	while true do
		...
		if getball then
			...
			if distance2 < 1.5 then
				...
				DeleteEntity(object)
				TriggerClientEvent(PlayerPedId(), GetHashKey("WEAPON_BALL"), 1, false, true)
				local GroupHandle = GetPlayerGroup(PlayerId())
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, 212) and GetLastInputMethod(2) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pet_menu') then
			OpenPetMenu()
		end
	end
end)
