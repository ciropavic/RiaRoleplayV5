...
ESX.RegisterServerCallback('eden_animal:buyPet', function(source, cb, pet)
	...
	if xPlayer.getMoney() >= price then
		...
		MySQL.Async.execute('UPDATE users SET pet = @pet WHERE identifier = @identifier', {
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text =_U('you_bought', pet, ESX.Math.GroupDigits(price))})
			cb(true)
		end)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = source, _U('your_poor')})
		cb(false)
	end
end)
