...
--KIMLIKLERI KARSI OYUNCUYA GOSTERIR
 ...
 if distance ~= -1 and distance <= 3.0 then
	...
	TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
        ...
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Yakında Oyuncu Yok.' })
	end
end, false)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, 3) and open or IsControlJustReleased(0, 25) and open or IsControlJustReleased(0, 177) and open  then
            SendNUIMessage({
                action = "close"
            })
            open = false
