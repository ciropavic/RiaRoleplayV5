...
blipshs = {}
RegisterNetEvent('mak:togglebarberblip')
AddEventHandler('mak:togglebarberblip',function()
    if not bliptrue then
        bliptrue = true
        for k, v in pairs(Config.Zones) do
            blipolustur(v, 71, 51, 0.7, 'Berber') -- 75 = Blip 4 = color 0.6 = blip scale oluyor
        end
    ...
    Citizen.CreateThread(function()
        while started do
            ...
            if (IsControlJustPressed(0, 215)) then
                ...
                DeletePed(Ped)
                cost = 0
            elseif (IsControlJustPressed(0, 344) or IsControlJustPressed(0, 311) or IsControlJustPressed(0, 57)) then
                started = false
                viewangle = true
