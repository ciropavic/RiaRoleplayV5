...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('http://becha_clothes/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://becha_clothes/togglecursor', JSON.stringify({}));
    })
    ...
    $('#savedright').on('click', function () {
        ...
        $.post('http://becha_clothes/loadsaved', JSON.stringify({
            "number": input.val()
        }));
    })
    ...
    $('#savemenu').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://becha_clothes/saveescape', JSON.stringify($(this).val()))
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://becha_clothes/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            $.post('http://becha_clothes/updateclothes', JSON.stringify({
                "name": nameId,
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://becha_clothes/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://becha_clothes/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://becha_clothes/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://becha_clothes/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://becha_clothes/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://becha_clothes/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://becha_clothes/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://becha_clothes/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://becha_clothes/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
