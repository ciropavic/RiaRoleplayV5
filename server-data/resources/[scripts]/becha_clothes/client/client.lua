...
RegisterNUICallback('saveescape', function(data, cb)
    ...
    TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = 'Kıyafeti aldınız ve 150$ ödediniz'})
    cb('ok')
end)
...
function OpenMenu(name)
    ...
    if isAllowed then
        ...
        exports['mythic_notify']:SendAlert('error', 'Burada istenmiyorsun!')
    end
end
...
Citizen.CreateThread(function()
	while true do
        ...
        if nearcloth < 5.0 then
            menu = {"clothesmenu", "Kıyafetlerini değiştirmek için ~r~E~s~ bas."}
        end
